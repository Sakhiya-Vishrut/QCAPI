...
namespace QCLorence.Common
{
    public static class TokenGenerator
    {
        ...
        public static string GenerateQCLorenceSecret()
        {
            using (SHA512 shaM = new SHA512Managed())
            {
                return Convert.ToBase64String(shaM.ComputeHash(Encoding.UTF8.GetBytes(ConfigItems.QCLorenceSecretKey)));
            }
        }

        public static string GenerateQCLorenceSecretAnonymous()
        {
            using (SHA512 shaM = new SHA512Managed())
            {
                return Convert.ToBase64String(shaM.ComputeHash(Encoding.UTF8.GetBytes(ConfigItems.QCLorenceSecretAnonymousKey)));
            }
        }
    }
}
